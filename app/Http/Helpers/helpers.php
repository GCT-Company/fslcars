...
function make_username($first_name, $last_name, $table = "users") {
    ...
    do {
        ...
        if ($chekUser == null) {
            ...
            $username_string_part = strrev($username_string_part); // username back to reverse
...
function filter_string_lower($string) {
    $username = preg_replace('/\s+/u', '', $string);
    $username = preg_replace('/[^\p{L}\p{N}\-]/u', '', $username); // Retain letters (including Arabic), numbers, and hyphens
    $username = mb_strtolower($username, 'UTF-8'); // Use mb_strtolower for multi-byte characters
    return $username;
}
