...
class CarTypeController extends Controller
{
    ...
    public function delete(Request $request)
    {
        ...
        // العثور على السيارة التي سيتم حذفها
        $carType = CarType::find($request->target);
    
        if (!$carType) {
            return back()->with(['error' => [__('Car type not found.')]]);
        }
    
        try {
            // حذف جميع التسعيرات المرتبطة بالسيارة
            CarPrice::where('car_id', $request->target)->delete();
    
            // حذف السيارة نفسها
            $carType->delete();
            
        } catch (\Exception $e) {
            // معالجة الأخطاء وإرجاع رسالة فشل
            return back()->with(['error' => [__('Something went wrong! Please try again.')]]);
        }
    
        // إرجاع رسالة نجاح
        return back()->with(['success' => [__('Car Type and its associated prices have been deleted successfully!')]]);
    }
