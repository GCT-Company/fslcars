...
class CarAreaController extends Controller
{
    ...
    public function delete(Request $request)
    {
        ...
        $car_area_id = $request->target;
    
        // ابحث عن المدينة باستخدام الـ ID
        $car_area = CarArea::find($car_area_id);
    
        // تحقق من وجود المدينة
        if (!$car_area) {
            return back()->with(['error' => [__('City not found!')]]);
        }
    
        try {
            // حذف جميع التسعيرات المرتبطة بالمدينة من جدول car_price
            CarPrice::where('from', $car_area_id)
                    ->orWhere('destination', $car_area_id)
                    ->delete();
