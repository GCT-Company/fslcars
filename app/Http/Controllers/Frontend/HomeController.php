...
class HomeController extends Controller
{
    ...
    public function index(BasicSettingsProvider $basic_settings)
    {
        ...
        // جلب بيانات 'from', 'destination' ونوع السيارة من جدول 'car_price' بدون تكرار
        // $carPrices = CarPrice::select(
        //     'car_prices.from',
        //     'car_prices.destination',
        //     'car_types.name as car_type_name',
        //     'from_area.name as from_area_name',
        //     'destination_area.name as destination_area_name'
        // )
        //     ->join('car_types', 'car_prices.car_id', '=', 'car_types.id')
        //     ->leftJoin('car_areas as from_area', 'car_prices.from', '=', 'from_area.id')
        //     ->leftJoin('car_areas as destination_area', 'car_prices.destination', '=', 'destination_area.id')
        //     ->distinct() // لضمان عدم تكرار السجلات
        //     ->get();


        // يمكنك استخدام $carPrices في العرض أو لأي غرض آخر

        $car_types  = CarType::where('status', true)->get();
        // dd($car_types);
        // أولاً، نحتاج لجلب القيم من عمود 'from' في جدول 'CarPrice'
        $fromValues = CarPrice::pluck('from')->toArray();

        // ثم نستخدم القيم التي حصلنا عليها للبحث في جدول 'CarArea'
        $areas = CarArea::whereIn('id', $fromValues)
            ->where('status', true)
            ->get();
        $fromValues = CarPrice::pluck('destination')->toArray();
        // ثم نستخدم القيم التي حصلنا عليها للبحث في جدول 'CarArea'
        $areas_destination = CarArea::whereIn('id', $fromValues)
            ->where('status', true)
            ->get();
        ...
        return view('frontend.index', compact('page_title', 'lang', 'default', 'banner', 'coloredText', 'remainingText', 'security', 'why_choose_us', 'footer', 'policies', 'testimonial', 'app', 'statistics', 'auth', 'policy', 'areas', 'car_types', 'areas_destination', 'cars', 'app_settings'));
    }
    ...
    public function findCarView()
    {
        ...
        $policies = SetupPage::orderBy('id')->where('type', $type)->where('status', 1)->get();
        $policy = SetupPage::orderBy('id')->where('type', $type)->where('status', 1)->where('slug', 'privacy-policy')->first();
        return view('frontend.pages.find-car', compact('page_title', 'cars', 'areas', 'types', 'app', 'footer', 'policy', 'policies', 'auth', 'default','areas_destination'));
    }
